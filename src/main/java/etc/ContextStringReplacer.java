 ...
 * found by matching against a given collection of substrings. The substrings
 * are used in the order of the collection to find a match. <br>
 * <br>
 * When iterating over a <code>ContextStringReplacer</code> object, a
 * <code>ContextStringReplacement</code> is provided for each found substring.
 * The <code>ContextStringReplacement</code> contains information about the
 * found substring as well as the characters before (lookbehind) and after
 * (lookahead) the found substring. <br>
 * <br>
 * The replacement for the found substring is then specified by calling the
 * <code>replace</code> method of the <code>ContextStringReplacement</code>
 * object. The replacement as part of the edited string is not searched for
 * matching substrings again. <br>
 * <br>
 * If <code>replace</code> is not called on the
 * <code>ContextStringReplacement</code> object, then the next substring is
 * searched beginning at the next character of the string. If the next substring
 * should be searched beginning after the found substring, then
 * <code>skip</code> can be called, which behaves like
 * <code>replace(getSubstring())</code>. <br>
 * <br>
 * The static <code>replace</code> method of the
 * <code>ContextStringReplacer</code> is added for convenience and allows to
 * express a replacement in one line of code. <br>
 * <br>
 ...
 * </pre>
 * 
 * Warning: <code>ContextStringReplacerIterator</code> and
 * <code>ContextStringReplacement</code> objects are single-use only and should
 * not be stored. <br>
 * <br>
...
public class ContextStringReplacer implements Iterable<ContextStringReplacer.ContextStringReplacerIterator.ContextStringReplacement> {
	...
	/**
	 * Create a new <code>ContextStringReplacer</code> for a string and a collection
	 * of strings to replace. The collection of strings to replace may not contain
	 * empty strings.
	 * 
	...
	/**
	 * Call <code>action</code> for every occurence of every substring in
	 * <code>replace</code> and return the result after all replacements
	 * 
	 * @param string  String to edit
	 * @param replace Collection of substrings to replace
	 * @param action  Action to be called with the found substrings, specifies the
	 *                replacement
	 * 
	 * @return Edited string
	 */
	public static String replace(String string, Collection<String> replace,
	...
	/**
	 * Iterator for the <code>ContextStringReplacer</code> class. Provides the
	 * <code>ContextStringReplacement</code> objects for each found occurence of a
	 * substring.
	 */
	public class ContextStringReplacerIterator implements Iterator<ContextStringReplacerIterator.ContextStringReplacement> {

		/**
		 * Create a new <code>ContextStringReplacerIterator</code> to iterate over all
		 * found substrings
		 */
		...
		/**
		 * Current index during the replacement. Everything before this index is already
		 * edited.
		 */
		...
		/**
		 * If this iterator is invalidated. The iterator is invalidated when a new
		 * iterator is created.
		 */
		...
		/**
		 * Find the next substring to replace and return the corresponding
		 * <code>ContextStringReplacement</code> object. Caches information in
		 * <code>nextReplacement</code>.
		 * 
		 * @return <code>ContextStringReplacement</code> object for the next substring
		 *         to replace
		 */
		private ContextStringReplacement findNext() {
			...
			// invalidate the current replacement (necessary to guarantee that the index
			// found here will be valid)
			if (currentReplacement != null)
				currentReplacement.invalidate();
		...
		/**
		 * Reset the cache for the next substring to replace. This causes
		 * <code>findNext</code> to actually search for the next substring instead of
		...
		/**
		 * Provides information about a found substring including its context and
		 * functionality to replace the found substring. <br>
		 * <br>
		 * Replacing or skipping the substring invalidates the information in the
		 * object, causing an IllegalStateException when calling any method after
		 * <code>replace</code>.
		 */
		public class ContextStringReplacement {
			...
			/**
			 * If this replacement is invalidated. The replacement is invalidated when
			 * replacing or skipping the found substring or moving on to a new replacement
			 * or iterator.
			 */
			private boolean invalid;
			...
			/**
			 * Get the lookahead of the found substring. If the part of the string after the
			 * found substring is too short for the requested size, then the lookahead
			 * contains all characters to the end of the string.
			 * 
			...
			/**
			 * Get the lookbehind of the found substring. If the part of the string before
			 * the found substring is too short for the requested size, then the lookbehind
			 * contains all characters from the beginning of the string.
			 */
			...
			/**
			 * Skip the replacement and continue after the found substring. This is
			 * different from not calling any method, which would continue the search at the
			 * next character. Calling this method invalidates the
			 * <code>ContextStringReplacement</code> object.
			 */
			public void skip() {
				if (invalid)
					throw new IllegalStateException();
			...
			/**
			 * Replace the found substring with the given replacement string. Calling this
			 * method invalidates the <code>ContextStringReplacement</code> object.
			 * 
